...
int main(int argc, const char* argv[])
{
    ...
    kj::Own<kj::NetworkAddress> addr    = network.parseAddress(argv[1]).wait(waitScope);  // NOLINT
...

    {
        ...
        KJ_ASSERT(response.getValue() == 123);  // NOLINT

        std::cout << "PASS" << std::endl;  // NOLINT
    }

    {
        ...
        auto response = readPromise.wait(waitScope);
        KJ_ASSERT(response.getValue() == 101);  // NOLINT
...

    {
        ...
        KJ_ASSERT(add3Promise.wait(waitScope).getValue() == 27);  // NOLINT
        KJ_ASSERT(add5Promise.wait(waitScope).getValue() == 29);  // NOLINT
...

    {
        ...
        KJ_ASSERT(fEvalPromise.wait(waitScope).getValue() == 1234);  // NOLINT
        KJ_ASSERT(gEvalPromise.wait(waitScope).getValue() == 4244);  // NOLINT
...

    {
        ...
        auto response = request.send().getValue().readRequest().send().wait(waitScope);
        KJ_ASSERT(response.getValue() == 512);  // NOLINT
